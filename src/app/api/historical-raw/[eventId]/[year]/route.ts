...
import { getHistoricalRawRounds } from '@/lib/api/datagolf/queries/historical-raw'
...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  if (numericYear < 2004 || numericYear > new Date().getFullYear()) {
  ...
  try {
    const results = await getHistoricalRawRounds(numericEventId, numericYear)
    ...
    console.error(`Error fetching raw rounds for event ${eventId} year ${year}:`, error)
    return NextResponse.json({ error: 'Failed to fetch historical round data' }, { status: 500 })
  }
}

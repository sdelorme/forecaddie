...
export function PlanSeasonTable({
  ...
  const hiddenCount = useMemo(() => events.filter((e) => hiddenSet.has(e.eventId)).length, [events, hiddenSet])
  ...
  const sortedEvents = useMemo(() => {
    const filtered = showHidden ? events : events.filter((e) => !hiddenSet.has(e.eventId))
  ...
  }, [events, sortKey, sortDir, showHidden, hiddenSet])
    ...
    <div className="space-y-2">
      {hiddenCount > 0 && (
        ...
        >
          ...
          {showHidden ? 'Hide' : 'Show'} {hiddenCount} hidden {hiddenCount === 1 ? 'event' : 'events'}
        </button>
      )}
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-800 border-gray-700 hover:bg-transparent">
              ...
              <TableHead
                ...
                className="text-gray-400 text-xs uppercase tracking-wider text-center w-[96px] cursor-pointer hover:text-gray-300 transition-colors select-none"
              >
              ...
              <TableHead className="text-gray-400 text-xs uppercase tracking-wider min-w-[180px]">Selected</TableHead>
              <TableHead className="text-gray-400 text-xs uppercase tracking-wider min-w-[160px]">Options</TableHead>
          ...
          <TableBody className="divide-y divide-gray-700/50">
            {sortedEvents.map((event) => {
              ...
              const isHidden = hiddenSet.has(event.eventId)
              const earnings =
              ...
              return (
                <TableRow
                  ...
                  className={cn(
                    'group bg-gray-800/50 border-gray-700/50 transition-colors cursor-pointer hover:bg-gray-700/60',
                ...
                >
                  ...
                  <TableCell className="py-2.5">
                    {hasLocked && locked ? (
                      ...
                      <span className="inline-flex items-center gap-2 rounded-md border border-dashed px-3 py-1.5 text-sm border-gray-600 text-gray-500 cursor-pointer hover:border-gray-500">
                  ...
                  <TableCell className="py-2 text-center">
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        onToggleHideEvent(event.eventId)
                      }}
                      className={cn(
